...
int main(void)
{
    ...
    //generating numbers
    int targetNumber;
    int* numbers = random_generator(PLAT_LAST, 1, 20, &targetNumber);
    ...
    while (!WindowShouldClose())
    {
        ...
        if (!gameOver && !gameWon) {
            ...
            if(IsKeyPressed(KEY_SPACE) && cube.Plat_index != -1){
                cube.vely = JUMP_FORCE;
            }

            // reveal, destroy platform and binary search logic
            if(IsKeyPressed(KEY_F) && cube.Plat_index != -1){
                ...
                if(cube.Plat_index != midIndex){
                    ...
                    if(plat[cube.Plat_index].value == targetNumber){
                        ...
                        for(int i = cube.Plat_index - 1; i >= searchStart; i--)    plat[i].revealed = 1;
                        searchStart = cube.Plat_index + 1;                        
                    } 
                    else{
                        for(int i = cube.Plat_index + 1; i <= searchEnd; i++)    plat[i].revealed = 1;
                        searchEnd = cube.Plat_index - 1;
                    }
                }
            }
        ...
        for(int i = 0; i < PLAT_LAST; i++){
            ...
            // map bottom collision
            if (cube.posy + cube.len >= WINDOW_HEIGHT) {
                cube.posy = WINDOW_HEIGHT - cube.len;  
                cube.vely = cube.velx = 0;                         
                cube.Plat_index = -1;                  
                gameOver = true;
            }
        }
            ...
            for(int i = PLAT_START; i < PLAT_LAST; i++){
                if(plat[i].existance){
                    if(!plat[i].revealed)
                        ...
                        DrawRectangleLinesEx((Rectangle){plat[i].posx, plat[i].posy, plat[i].len, plat[i].width}, 5, ORANGE);
                        char num_text[16];
